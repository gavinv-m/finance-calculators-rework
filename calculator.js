...
function calculateHouseFlip() {
  ...
  let downPaymentBase = downPaymentType === "purchaseAndReno"
    ? (purchase + reno)
    : purchase;
  ...
  let loanAmount = downPaymentType === "purchaseAndReno"
  ...
  let gapFundingFees = gapCosts > 0
    ? (gapCosts * gapFundingRate) / 100
    : 0;

  let totalInvestment = purchase + reno + holding + loanInterest + loanFees + gapFundingFees + proratedTaxes + proratedInsurance;
  ...
  let breakEvenYears = monthlyRentalProfit > 0 ?
    (netProfit / (monthlyRentalProfit * 12)).toFixed(2) : "N/A";
  ...
  let rentalVsFlip = (netProfit > 0 && monthlyRent > 0)
  ...
  let requiredARV = desiredProfitMargin > 0
...
function calculateRetirement() {
  ...
  // ✅ Validation function
  function validateInput(input, errorField, fieldName, min = 0, max = Infinity) {
  ...
  let fvCurrentSavings =
    (parseFloat(currentSavings.value) || 0) * Math.pow(1 + (parseFloat(annualReturn.value) || 0) / 100, yearsToRetirement);

  let fvContributions =
    (parseFloat(monthlyContributions.value) || 0) * ((Math.pow(1 + r, t) - 1) / r) * (1 + r);

  // ✅ Stock and Real Estate appreciation (Avoid NaN)
  let fvStock =
    (parseFloat(currentStockValue.value) || 0) * Math.pow(1 + 0.03, yearsToRetirement);
  ...
  let adjustedIncome =
    (parseFloat(desiredIncome.value) || 0) * Math.pow(1 + (parseFloat(inflationRate.value) || 0) / 100, yearsToRetirement);
  ...
  for (let i = 5; i <= yearsToRetirement; i += 5) {
    ...
    const remainingMortgage = principal * Math.pow(1 + monthlyRate, monthsPaid) -
